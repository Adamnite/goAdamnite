...
type Peer struct {
	...
	*bargossip.Peer
	...
	blockHead bytes.Hash // head block hash
	...
	txpool      TxPool
	knownTxs    gset.Set
	txBroadcast chan []bytes.Hash
	txAnnounce  chan []bytes.Hash
...
func NewPeer(version uint, p *bargossip.Peer, rw bargossip.MsgReadWriter, txpool TxPool) *Peer {
	peer := &Peer{
		...
		txBroadcast:         make(chan []bytes.Hash),
		txAnnounce:          make(chan []bytes.Hash),
		txpool:              txpool,
...
func (p *Peer) Head() (hash bytes.Hash) {
	p.lock.RLock()
