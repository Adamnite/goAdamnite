...
type Signer interface {
	...
	Sender(tx *Transaction) (bytes.Address, error)
	...
	Hash(tx *Transaction) bytes.Hash
}

type AdamniteSigner struct{}

func (as AdamniteSigner) Sender(tx *Transaction) (bytes.Address, error) {
	if tx.Type() != VOTE_TX && tx.Type() != NORMAL_TX {
		return bytes.Address{}, ErrTxTypeNotSupported
	}
	v, r, s := tx.RawSignature()
	return recoverPlain(as.Hash(tx), r, s, v)
}

func (as AdamniteSigner) Hash(tx *Transaction) bytes.Hash {
...
func recoverPlain(sighash bytes.Hash, R, S, Vb *big.Int) (bytes.Address, error) {
	if Vb.BitLen() > 8 {
		return bytes.Address{}, ErrInvalidSig
	}
	V := byte(Vb.Uint64() - 27)
	...
	if len(pub) == 0 || pub[0] != 4 {
		return bytes.Address{}, errors.New("invalid public key")
	}
...
func Sender(signer Signer, tx *Transaction) (bytes.Address, error) {
