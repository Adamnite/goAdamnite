...
func VerifyProof(rootHash utils.Hash, key []byte, proofDb adamnitedb.AdamniteDBReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash utils.Hash, root node, key []byte, proofDb adamnitedb.AdamniteDBReader, allowNonExistent bool) (node, []byte, error) {
	// resolveNode retrieves and resolves trie node from merkle proof stream
	resolveNode := func(hash utils.Hash) (node, error) {
	...
	for {
		...
		case *fullNode:
			...
			continue
		case hashNode:
			child, err = resolveNode(utils.BytesToHash(cld))
			if err != nil {
				return nil, nil, err
			}
		case valueNode:
...
func VerifyRangeProof(rootHash utils.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof adamnitedb.AdamniteDBReader) (bool, error) {
