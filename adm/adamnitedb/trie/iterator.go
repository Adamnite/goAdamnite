...
type NodeIterator interface {
	...
	AddResolver(adamnitedb.Database)
}

// nodeIteratorState represents the iteration state at one particular node of the
// trie, which can be resumed at a later invocation.
type nodeIteratorState struct {
	hash    bytes.Hash // Hash of the node being iterated (nil if not standalone)
	node    node        // Trie node being iterated
	parent  bytes.Hash // Hash of the first full ancestor node (nil if current is the root)
...
type nodeIterator struct {
	...
	resolver adamnitedb.Database // Optional intermediate resolver above the disk layer
}
...
func (it *nodeIterator) AddResolver(resolver adamnitedb.Database) {
	it.resolver = resolver
}

func (it *nodeIterator) Hash() bytes.Hash {
	if len(it.stack) == 0 {
		return bytes.Hash{}
	}
...
func (it *nodeIterator) Parent() bytes.Hash {
	if len(it.stack) == 0 {
		return bytes.Hash{}
	}
...
func findChild(n *fullNode, index int, path []byte, ancestor bytes.Hash) (node, *nodeIteratorState, []byte, int) {
	var (
		child     node
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor bytes.Hash) (*nodeIteratorState, []byte, bool) {
...
func (it *nodeIterator) nextChildAt(parent *nodeIteratorState, ancestor bytes.Hash, key []byte) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() bytes.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() bytes.Hash {
	return it.b.Parent()
}
...
func (it *differenceIterator) AddResolver(resolver adamnitedb.Database) {
...
func (it *unionIterator) Hash() bytes.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() bytes.Hash {
...
func (it *unionIterator) AddResolver(resolver adamnitedb.Database) {
	panic("not implemented")
}
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == bytes.Hash{}) {
			it.count++
