...
func stackTrieFromPool(db adamnitedb.AdamniteDBWriter) *StackTrie {
...
type StackTrie struct {
	...
	db        adamnitedb.AdamniteDBWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db adamnitedb.AdamniteDBWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func (st *StackTrie) setDb(db adamnitedb.AdamniteDBWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(ko int, key, val []byte, db adamnitedb.AdamniteDBWriter) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db adamnitedb.AdamniteDBWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
