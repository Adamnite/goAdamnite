...
func newContractHeld(contract *bytes.Address, db VM.DBInterfaceItem) (*contractHeld, error) {
...
func (csh *ContractStateHolder) RunOnUntil(state *statedb.StateDB, transactionCount int, reasonToQuit <-chan any, workingBlock *utils.VMBlock) (transactions []utils.TransactionType, err error) {
	...
	if workingBlock != nil {
		workingBlock.BalanceChanges[bytes.Address{}] = big.NewInt(0)
	...
	successes.ForEach(func(i int, a any) bool {
		...
		if workingBlock != nil {
			...
			workingBlock.BalanceChanges[bytes.Address{}] = big.NewInt(0).Add(workingBlock.BalanceChanges[bytes.Address{}], gasUsed)
		}
