...
func newTable(t transport, db *enode.DB, bootnodes []*enode.Node, log log15.Logger) (*Table, error) {
	tab := &Table{
		net:        t,
		db:         db,
...
func (tab *Table) loadSeedNodes() {
	...
	for i := range seeds {
		seed := seeds[i]
		age := log15.Lazy{Fn: func() interface{} { return time.Since(tab.db.LastPongReceived(seed.ID(), seed.IP())) }}
