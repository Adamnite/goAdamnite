...
func NewLocalNode(db *DB, key *ecdsa.PrivateKey) *LocalNode {
	localNode := &LocalNode{
		...
		endpoint4: localNodeEndpoint{track: netutil.NewIPTracker(iptrackWindow, iptrackContactWindow, iptrackMinStatements)},
		endpoint6: localNodeEndpoint{track: netutil.NewIPTracker(iptrackWindow, iptrackContactWindow, iptrackMinStatements)},
	}

	localNode.seq = db.localSeq(localNode.id)
...
func (ln *LocalNode) Node() *Node {
	...
	defer ln.mu.Unlock()
	ln.sign()
	return ln.currentNode.Load().(*Node)
}
...
func (ln *LocalNode) endpointForIP(ip net.IP) *localNodeEndpoint {
	if ip.To4() != nil {
		return &ln.endpoint4
	}
	return &ln.endpoint6
...
func (e *localNodeEndpoint) get() (newIP net.IP, newPort int) {
...
func (ln *LocalNode) invalidate() {
	ln.currentNode.Store((*Node)(nil))
}

func (ln *LocalNode) sign() {
	if n := ln.currentNode.Load().(*Node); n != nil {
		return // no changes
	}
	...
	ln.currentNode.Store(n)
	log15.Info("New local node record", "seq", ln.seq, "id", n.ID(), "ip", n.IP(), "udp", n.UDP(), "tcp", n.TCP())
}
