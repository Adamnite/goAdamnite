...
func (db *DB) fetchInt64(key []byte) int64 {
	blob, err := db.levelDB.Get(key, nil)
	if err != nil {
		return 0
	}
...
func (db *DB) storeInt64(key []byte, n int64) error {
	...
	return db.levelDB.Put(key, blob, nil)
}
...
func (db *DB) Node(id ID) *Node {
	blob, err := db.levelDB.Get(nodeKey(id), nil)
	if err != nil {
		return nil
	}
...
func (db *DB) UpdateNode(node *Node) error {
	...
	if err := db.levelDB.Put(nodeKey(node.ID()), blob, nil); err != nil {
		return err
	}
...
func (db *DB) DeleteNode(id ID) {
	deleteRange(db.levelDB, nodeKey(id))
}
...
func (db *DB) expireNodes() {
	it := db.levelDB.NewIterator(util.BytesPrefix([]byte(dbNodePrefix)), nil)
	defer it.Release()
	if !it.Next() {
		return
	}
	...
	for !atEnd {
		...
		if field == dbNodePong {
			...
			if time < threshold {
				...
				deleteRange(db.levelDB, nodeItemKey(id, ip, ""))
			}
		}
		atEnd = !it.Next()
