...
func testPeer(protos []Protocol) (func(), *conn, *Peer, <-chan error) {
	...
	peer := newPeer(log15.Root(), c1, protos)
...
func TestNewPeer(t *testing.T) {
	name := "nodename"
	caps := []PeerCapability{{"foo", 2}, {"bar", 3}}
	id := randomID()
...
func TestMatchProtocols(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Remote: []PeerCapability{{Name: "local"}, {Name: "match1"}, {Name: "match2"}},
		...
		},
		{
			...
			Remote: []PeerCapability{{Name: "aa"}, {Name: "ab"}, {Name: "bb"}, {Name: "ba"}},
