...
func main() {
	app := cli.NewApp()
	app.Name = "bootnode"
	app.Usage = "CLI application for the go Adamnite bootnode"
	app.Version = "1.0.0"

	app.Flags = []cli.Flag{}

	app.Flags = flags.Merge(app.Flags, debug.Flags)

	app.Before = func(ctx *cli.Context) error {
		utils.ShowBootNodeBanner()
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(flags.DeveloperFlag.Name):
		log.Info("Starting bootnode on Adamnite devnet ...")
		log.Debug("All blockchain datas will be stored on the memory db.")
	case ctx.IsSet(flags.MainnetFlag.Name):
		log.Info("Starting bootnode on Adamnite mainnet ...")
	case ctx.IsSet(flags.TestnetFlag.Name):
		log.Info("Starting bootnode on Adamnite testnet ...")
	default:
		log.Info("Starting bootnode on Adamnite mainnet ...")
	}
}

func startBootNode(ctx *cli.Context) error {
	log.Info("Adamnite bootnode starting...")
