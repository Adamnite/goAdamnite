...
import (
	...
	"github.com/adamnite/go-adamnite/metrics/prometheus"
)
...
func Setup(address string) {
	...
	log15.Info("Starting metrics server", "addr", fmt.Sprintf("http://%s/debug/metrics", address))
	go func() {
		if err := http.ListenAndServe(address, m); err != nil {
			log15.Error("Failure in running metrics server", "err", err)
		}
	}()
}
