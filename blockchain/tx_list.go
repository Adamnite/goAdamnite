...
func (l *txList) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	if old != nil {
		...
		a = a.Mul(a, old.InnerData.ATE_price())
		b := big.NewInt(100)
		threshold := a.Div(a, b)
		// Have to ensure that the new gas price is higher than the old gas
		// price as well as checking the percentage threshold to ensure that
		// this is accurate for low (Wei-level) gas price replacements
		if old.ATEPriceCmp(tx) >= 0 || tx.ATEPriceIntCmp(threshold) < 0 {
			return false, nil
		}
	}
...
func (l *txList) Filter(costLimit *big.Int, gasLimit uint64) (types.Transactions, types.Transactions) {
	...
	removed := l.txs.Filter(func(tx *types.Transaction) bool {
		return tx.ATEMax() > gasLimit || tx.Cost().Cmp(costLimit) > 0
	})
...
func (l *txPricedList) Cap(threshold *big.Int) types.Transactions {
	...
	for len(*l.remotes) > 0 {
		...
		if cheapest.ATEPriceIntCmp(threshold) >= 0 {
			break
		}
		heap.Pop(l.remotes)
...
func (l *txPricedList) Underpriced(tx *types.Transaction) bool {
	...
	return cheapest.ATEPriceCmp(tx) >= 0
}
...
func (l *txPricedList) Reheap() {
	...
	l.all.Range(func(hash utils.Hash, tx *types.Transaction, local bool) bool {
