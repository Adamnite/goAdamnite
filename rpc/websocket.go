...
func (s *Server) WebsocketHandler(allowedOrigins []string) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log15.Debug("WebSocket upgrade failed", "err", err)
			return
		}
...
func wsHandshakeValidator(allowedOrigins []string) func(*http.Request) bool {
	...
	log15.Debug(fmt.Sprintf("Allowed origin(s) for WS RPC interface %v", origins.ToSlice()))
	...
	f := func(req *http.Request) bool {
		...
		log15.Warn("Rejected WebSocket connection", "origin", origin)
		return false
	}
...
func ruleAllowsOrigin(allowedOrigin string, browserOrigin string) bool {
	...
	if err != nil {
		log15.Warn("Error parsing allowed origin specification", "spec", allowedOrigin, "error", err)
		return false
	}
	browserScheme, browserHostname, browserPort, err = parseOriginURL(browserOrigin)
	if err != nil {
		log15.Warn("Error parsing browser 'Origin' field", "Origin", browserOrigin, "error", err)
		return false
	}
...
func DialWebsocketWithDialer(ctx context.Context, endpoint, origin string, dialer websocket.Dialer) (*AdamniteClient, error) {
...
func DialWebsocket(ctx context.Context, endpoint, origin string) (*AdamniteClient, error) {
