...
type ChainHeaderReader interface {
	...
	GetHeaderByHash(hash bytes.Hash) *types.BlockHeader

	GetHeader(hash bytes.Hash, number *big.Int) *types.BlockHeader
}
...
type ChainReader interface {
	...
	GetBlockByHash(hash bytes.Hash) *types.Block
...
type Engine interface {
	...
	Witness(header *types.BlockHeader) (bytes.Address, error)
