...
type DposData struct {
	...
	Votes map[utils.Address]utils.Voter `json:"votes"`
}
...
func (adpos *AdamniteDPOS) witnesspool(chain ChainReader, number *big.Int, hash utils.Hash, parents []*types.BlockHeader) (*WitnessPool, error) {
...
func (adpos *AdamniteDPOS) dbwitnesspool(chain ChainReader, number *big.Int, hash utils.Hash, parents []*types.BlockHeader) (*poh.DBWitnessPool, error) {
	// Search for a snapshot in memory or on disk for checkpoints
...
func (adpos *AdamniteDPOS) Witness(header *types.BlockHeader) (utils.Address, error) {
	return header.Witness, nil
}

func (adpos *AdamniteDPOS) DBWitness(header *types.BlockHeader) (utils.Address, error) {
...
func (adpos *AdamniteDPOS) calVote(chain ChainReader, header *types.BlockHeader, state *statedb.StateDB, txs []*types.Transaction) (votes map[utils.Address]utils.Voter) {
	votes = map[utils.Address]utils.Voter{}
